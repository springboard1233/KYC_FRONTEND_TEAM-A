...
const Title = ({level, children, className}) => {
    ...
    // return React.createElement(Heading, { className: `${className} ${classes}` }, children);
...
const CustomNavLinkList = ({href, className, isActive, children}) => {
    const linkStyles= "text-[17px] font-medium cursor-pointer list-none hover:text-[#cdadf0] transition-all ease-in-out";
...
const PrimaryButton =({children, className})=>{
    return(
        <button type='submit' className={`${className} bg-green text-white font-medium rounded-full text-lg px-16 py-3 hover:bg-[#7550d0] transition-all ease-in-out`}>
            {children}
        </button>
    );
};
...
const Heading =({title, subtitle, subtitleClassName = ""}) =>{
    return (
        <>
            <Title level={4}>{title}</Title>
            <div className="w-1/2">
                <Caption className={subtitleClassName}>{subtitle}</Caption>
            </div>
        </>
    );
};
